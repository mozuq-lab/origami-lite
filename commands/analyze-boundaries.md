...
**Phase 3（analyze-boundaries）の入力制限**:
...
**コンテキスト分離のポイント**:
- Phase 3は `02_動作仕様.md` **のみ**を参照（`01_機能詳細.md` は参照しない）
- Must/Never項目から境界値を特定
- 🔴項目がある場合は、Phase 3実行前に修正推奨

### 事前チェック（タスク分割経由時） 🟡

タスク分割経由時（`--target` 指定時）:
1. `{機能ディレクトリ}/02_動作仕様.md` の存在を確認
2. 存在しない場合はエラー:
   ```
   エラー: 入力ファイルが見つかりません: {パス}/02_動作仕様.md
...
**移行時の注意**:
- Phase 3 は `01_機能詳細.md` を参照しません
- 必要な情報は Phase 2 の出力（`02_動作仕様.md`）に含まれている必要があります
