---
description: 動作仕様と境界値分析に基づいて、実行可能なテストケース一覧を生成します。タスク分割経由時はテストケース、単独実行時はテストケース一覧を出力します。
---
...
**実行コンテキストに応じたファイル出力**:
- **単独実行時**: 複数機能を一覧形式で `04_テストケース一覧.md` に出力（後方互換）
- **タスク分割経由時**（`--target` 指定）: 単一機能のテストケースを `04_テストケース.md` に出力
...
2. **前工程出力の確認**
   ...
   - `{出力先}/03_境界値分析表.md` が存在する場合は読み込み
   - `{出力先}/01_機能一覧.md` が存在する場合は読み込み（補助情報）
...
- **入力**:
  ...
  - `{出力先ディレクトリ}/02_動作仕様.md`
  - `{出力先ディレクトリ}/03_境界値分析.md`
- **重要**: 他の機能フォルダ（F-001_xxx/, F-002_xxx/ 等）のファイルは参照しない
- これにより、コンテキストサイズが一定に保たれます

### フェーズ間入力制限（v3.1.0） 🔵

**Phase 4（generate-cases）の入力制限**:
...
**コンテキスト分離のポイント**:
- Phase 4は `01`, `02`, `03` の3ファイルを参照（仕様書は参照しない）
- 前3フェーズの出力品質がテストケース品質に直結
- 🔴項目がある場合は、テスト実行前に確認必須
...
Phase 4はフェーズ間入力制限の**最終段階**です：

- **入力品質の重要性**: 01〜03の🔴項目は最終テストケースに影響
- **レビュー推奨**: Phase 4実行前に01〜03をレビュー
- **🔴項目の扱い**: テストケースに継承された🔴項目はステークホルダー確認必須
...
- **単独実行時**: `{出力先}/04_テストケース一覧.md`（複数機能を一覧形式で出力）
- **タスク分割経由時**: `{出力先}/04_テストケース.md`（単一機能のテストケースを出力）
...
- 出力ファイルが作成されたことを確認
  - 単独実行時: `{出力先}/04_テストケース一覧.md`
  - タスク分割経由時: `{出力先}/04_テストケース.md`
- 以下のサマリーを表示：
...
**移行時の注意**:
- Phase 4 は仕様書を直接参照しません
- 必要な情報は Phase 1〜3 の出力に含まれている必要があります
- Phase 4 は唯一、複数ファイル（3ファイル）を入力とするフェーズです
- 単独実行モード（`--target` なし）の動作は変更ありません
